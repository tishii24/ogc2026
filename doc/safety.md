...
### 3. `solver.rs`: `apply_y_event()` の内部不変条件依存 unwrap
...
### 4. `collision.rs`: block/orientation index の `expect()` / 添字アクセス
...
### 5. `solver.rs`: `pref_penalty[s.block_id][s.bay_id]`
...
### 7. `util.rs` の未使用 sampler 系 assert
...
3. 必要なら `validate_problem()` を追加して、入力形式不整合を `Err(String)` にする。
