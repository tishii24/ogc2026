問題文・解法コード・実行ログを確認し、評価関数、近傍、差分計算、時間配分、実装上の不整合を分けてレビューします。まず全体構成と問題制約を把握します。

次に、現行アルゴリズムの構成、パラメータ、ログの種類を特定します。レビューはコード上の理論的評価と、ログに現れている実測傾向を照合して行います。

コード量とログ量が多いため、幾何・探索・正当性・実測分析を独立に精査し、最後に優先順位付きで統合します。変更は行わず、根拠となる箇所と期待効果を明確にします。

重大度の高い指摘（初期解未出力、時刻表現、preoptimize容量近似、近傍時間配分）を現行コードで再確認します。あわせて、ログから見える改善余地を「品質／反復効率／安全性」に分けて優先順位化します。

探索空間を狭めている箇所と、未処理・境界条件・固定化された仮定を横断検索します。ここでは「実行不能化するバグ」と「安全側だがスコアを落とす制約」を分けます。

最後に、`prob_40` のフェーズ間でどこまでスコアが失われ／回収されているかを確認し、短時間の tardiness 改善策を具体化します。
